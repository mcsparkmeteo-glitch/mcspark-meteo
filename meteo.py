...
f"http://api.weatherapi.com/v1/forecast.json?key={API_KEY}&q={lat_float},{lon_float}&days=2&aqi=yes&alerts=no"
...
"Discreta" if base_pm10 < 35 else "Scadente" if base_pm10 < 50
...
f"https://marine-api.open-meteo.com/v1/marine?latitude={','.join(mar_lats)}&longitude={','.join(mar_lons)}&hourly=wave_height,sea_surface_temperature&forecast_days=1&timezone=Europe/Rome"
...
for i, (nome_mare, coord) in enumerate(DIZIONARIO_MARI.items()):
    ...
    altezza_onda = pt_marine["hourly"]["wave_height"][14] if

(isinstance(pt_marine, dict) and "hourly" in pt_marine) else 0.4


    temp_mare = pt_marine["hourly"]["sea_surface_temperature"][14] if

(isinstance(pt_marine, dict) and "hourly" in pt_marine) else 18.5
    ...
    hex_m = "d73027" if temp_mare >= 24 else "fee090" if temp_mare

>= 19 else "e0f3f8" if temp_mare >= 14 else "4575b4"


    dati_mari_render.append({"nome": nome_mare, "lat": coord["lat"],

"lon": coord["lon"], "icona": icona_mare, "temp": round(temp_mare, 1),

"testo": stato_testo, "colore_classe": hex_m})
...
def tabella_regionale_pioggia(regione_nome):
    ...
    html = f"<div class='scheda-meteo s-pioggia'><h3

style='margin:0 0 10px 0; color:#1f77b4; border-bottom:2px solid

#1f77b4; padding-bottom:5px; font-size:14px;

text-align:center;'>{regione_nome}: Precipitazioni 24h</h3>"


    html += "<table style='width:100%; border-collapse:collapse;

text-align:center; font-size:11px; border:1px solid #ddd;'><tr

style='background-color:#f8f9fa; font-weight:bold;


height:26px;'><td>Provincia</td><td>GFS</td><td>ICON</td><td>ECMWF</td><td

style='background-color:#e6f2ff;'>MEDIA</td></tr>"
...
height:26px;'><td><b>{p_nome}{' ⚡' if d['fulmini'] else


''}</b></td><td>{d['pioggia']['gfs']}</td><td>{d['pioggia']['icon']}</td><td>{d['pioggia']['ecmwf']}</td><td

style='background-color:#e6f2ff; font-weight:bold;

color:#1f77b4;'>{d['pioggia']['media']} mm</td></tr>"
...
def tabella_regionale_temperatura(regione_nome):
    ...
    html = f"<div class='scheda-meteo s-temp'

style='display:none;'><h3 style='margin:0 0 10px 0; color:#d62728;

border-bottom:2px solid #d62728; padding-bottom:5px; font-size:14px;

text-align:center;'>{regione_nome}: Temperature Medie</h3>"
...
height:26px;'><td>Provincia</td><td

style='color:blue;'>07:00</td><td

style='color:red;'>14:00</td><td

style='color:darkblue;'>22:00</td></tr>"
...
''}</b></td><td style='color:blue;'>{d['t7']['media']}

°C</td><td style='color:red; font-weight:bold;

background-color:#ffe6e6;'>{d['t14']['media']} °C</td><td

style='color:darkblue;'>{d['t22']['media']} °C</td></tr>"
...
def tabella_regionale_vento(regione_nome):
    ...
    html = f"<div class='scheda-meteo s-vento'

style='display:none;'><h3 style='margin:0 0 10px 0; color:#ff7f0e;

border-bottom:2px solid #ff7f0e; padding-bottom:5px; font-size:14px;

text-align:center;'>{regione_nome}: Venti e Raffiche</h3>"
...
style='background-color:#ffe6cc;'>MEDIA</td></tr>"
...
''}</b></td><td>{d['vento']['gfs']}</td><td>{d['vento']['icon']}</td><td>{d['vento']['ecmwf']}</td><td

style='background-color:#ffe6cc; font-weight:bold;

color:#ff7f0e;'>{d['vento']['media']} km/h
...
def tabella_regionale_smog(regione_nome):
    ...
    html = f"<div class='scheda-meteo s-smog'

style='display:none;'><h3 style='margin:0 0 10px 0; color:#9467bd;

border-bottom:2px solid #9467bd; padding-bottom:5px; font-size:14px;

text-align:center;'>{regione_nome}: Qualità dell'Aria</h3>"
...
height:26px;'><td>Provincia</td><td>PM10</td><td

style='background-color:#f3e6ff;'>Stato Aria</td></tr>"
...
''}</b></td><td>{d['smog']['valore']}

µg/m³</td><td style='background-color:#f3e6ff;

font-weight:bold;'>{d['smog']['giudizio']}</td></tr>"
...
for d in dati_render_mappa:
    ...
    hex_p = "001d58" if d["pioggia"]["media"] >= 15 else "225ea8" if

d["pioggia"]["media"] >= 5 else "41b6c4" if d["pioggia"]["media"]

>= 1 else "a1dab4"


    folium.Circle(location=[d["lat"], d["lon"]], radius=raggio_mappa,

color="transparent", weight=0, fill=True, fill_opacity=0.7,

className=f"v-filtro v-pioggia sfumatura-{hex_p}").add_to(map_italia)
...
className=f"v-filtro v-temp sfumatura-{hex_t}").add_to(map_italia)
...
className=f"v-filtro v-vento sfumatura-{hex_w}").add_to(map_italia)
...
className=f"v-filtro v-smog sfumatura-{hex_s}").add_to(map_italia)
...
for mare in dati_mari_render:
    ...
    folium.Circle(location=[mare["lat"], mare["lon"]], radius=110000,

color="transparent", weight=0, fill=True, fill_opacity=0.55,

className=f"sfumatura-{mare['colore_classe']}").add_to(map_italia)


    popup_html = f"<div style='font-family: Arial, sans-serif;

font-size:11px; width:180px;'><h4 style='margin:0 0 5px 0;

color:#005580; border-bottom:1px solid #ccc;

padding-bottom:3px;'>{mare['nome']}</h4><b>Moto

Ondoso:</b> {mare['testo']}<br><b>Temp.

Acqua:</b> <b>{mare['temp']} °C</b></div>"


    folium.Marker(location=[mare["lat"], mare["lon"]],

icon=folium.DivIcon(html=f"<div style='font-family: Arial,

sans-serif; font-size: 11px; text-align: center; font-weight: bold;

color: #003366; text-shadow: 1px 1px 2px white;'><span

style='font-size:16px;'>{mare['icona']}</span><br>🌡️

{mare['temp']}°C</div>"), popup=folium.Popup(popup_html,

max_width=220)).add_to(map_italia)
...
<style>
 ...
 #pannello-meteo-pulsanti { position: fixed; top: 20px; right: 20px;

background: white; padding: 12px; border: 2px solid #2c3e50;

border-radius: 8px; z-index: 9999; font-family: Arial, sans-serif;

box-shadow: 4px 4px 15px rgba(0,0,0,0.2); width: 320px; box-sizing:

border-box; }


    #pannello-meteo-pulsanti h4 { margin: 0 0 10px 0; font-size: 13px;

color: #2c3e50; border-bottom: 2px solid #2c3e50; padding-bottom: 5px;
...
<svg width="0" height="0"><defs>
    ...
    <radialGradient id="grad-a1dab4"><stop offset="0%"

stop-color="#a1dab4" stop-opacity="0.8"/><stop offset="100%"

stop-color="#a1dab4" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-41b6c4"><stop offset="0%"

stop-color="#41b6c4" stop-opacity="0.85"/><stop offset="100%"

stop-color="#41b6c4" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-225ea8"><stop offset="0%"

stop-color="#225ea8" stop-opacity="0.9"/><stop offset="100%"

stop-color="#225ea8" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-001d58"><stop offset="0%"

stop-color="#001d58" stop-opacity="0.95"/><stop offset="100%"

stop-color="#001d58" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-1a9850"><stop offset="0%"

stop-color="#1a9850" stop-opacity="0.8"/><stop offset="100%"

stop-color="#1a9850" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-fee08b"><stop offset="0%"

stop-color="#fee08b" stop-opacity="0.8"/><stop offset="100%"

stop-color="#fee08b" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-f46d43"><stop offset="0%"

stop-color="#f46d43" stop-opacity="0.85"/><stop offset="100%"

stop-color="#f46d43" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-d73027"><stop offset="0%"

stop-color="#d73027" stop-opacity="0.95"/><stop offset="100%"

stop-color="#d73027" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-e0f3f8"><stop offset="0%"

stop-color="#e0f3f8" stop-opacity="0.8"/><stop offset="100%"

stop-color="#e0f3f8" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-fee090"><stop offset="0%"

stop-color="#fee090" stop-opacity="0.85"/><stop offset="100%"

stop-color="#fee090" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-66bd63"><stop offset="0%"

stop-color="#66bd63" stop-opacity="0.8"/><stop offset="100%"

stop-color="#66bd63" stop-opacity="0"/></radialGradient>


    <radialGradient id="grad-4575b4"><stop offset="0%"

stop-color="#4575b4" stop-opacity="0.85"/><stop offset="100%"

stop-color="#4575b4" stop-opacity="0"/></radialGradient>
...
<div id="sidebar-tabelle-mcspark">
    ...
    <div id="contenitore-vuoto-sidebar">
        ...
        <div class="messaggio-benvenuto-sidebar">
            ...
            <b>Clicca direttamente sul cerchietto vicino alla

regione</b> per vederne istantaneamente i dati provinciali

completi qui a sinistra.
...
<div id="pannello-meteo-pulsanti">
    ...
    <label class="opzione-radio"><input type="radio"

name="filtro-global" value="pioggia" checked>🌧️ Pioggia Prevista
...
name="filtro-global" value="temp">🌡️ Temperature
...
name="filtro-global" value="vento">💨 Vento e Raffiche</label>


    <label class="opzione-radio"><input type="radio"

name="filtro-global" value="smog">😷 Qualità dell'Aria
...
branding_html = (
    ...
    f'<div style="position: fixed; bottom: 35px; right: 20px; width:

60px; height: auto; z-index: 9999; border: 1px solid #2c3e50;

border-radius: 4px; overflow: hidden; box-shadow: 0 2px 5px
...
rgba(0,0,0,0.2);">'
    ...
    f'<div style="position: fixed; bottom: 10px; left: 50%;

transform: translateX(-50%); background-color: rgba(255,255,255,0.9);

padding: 4px 10px; border-radius: 4px; z-index: 9999; font-family:

Arial, sans-serif; font-size: 10px; color: #333; border: 1px solid #ccc;
...
white-space: nowrap;">'
    ...
    f'  <b>{COPYRIGHT}</b> | <span

style="color:#666;">{FONTE_DATI}</span> | <span

style="color:#005580;

font-weight:bold;">{STRINGA_AGGIORNAMENTO}</span>'
