...
legenda_data_html = f'''
<div style="position: absolute !important; top: 220px !important; left: 350px !important; background-color: rgba(200, 0, 0, 0.85); color: white; padding: 6px 16px; border-radius: 20px; z-index: 9999 !important; font-family: Arial, sans-serif !important; font-size: 14px !important; font-weight: bold; border: 2px solid white; box-shadow: 0px 3px 8px rgba(0,0,0,0.4); pointer-events: none;">
...
for r_nome, coord in REGIONI_COORDINATE.items():
    ...
    <div onclick="mostraRegioneLaterale('{id_pulito}')" title="{r_nome}" style="width: 24px; height: 24px; background-color: white; border: 3px solid #2c3e50; border-radius: 50%; cursor: pointer; box-shadow: 0px 2px 6px rgba(0,0,0,0.3); transform: translate(-12px, -12px); z-index: 99999;"></div>
    """
...
function aggiornaMappaEInvolucri(valoreFiltro) {
    ...
    for (var i = 0; i < tuttiIFiltri.length; i++) { tuttiIFiltri[i].classList.remove('v-attivo'); }
    var filtriDaAttivare = document.getElementsByClassName('v-' + valoreFiltro);
    for (var j = 0; j < filtriDaAttivare.length; j++) { filtriDaAttivare[j].classList.add('v-attivo'); }
...
for (var i = 0; i < radioButtons.length; i++) { radioButtons[i].addEventListener('change', function(e) { aggiornaMappaEInvolucri(e.target.value); }); }
...
<div style="position: fixed; bottom: 10px; left: 50%; transform: translateX(-50%); background-color: rgba(255,255,255,0.9); padding: 4px 10px; border-radius: 4px; z-index: 9999; font-family: Arial, sans-serif; font-size: 10px; color: #333; border: 1px solid #ccc; box-shadow: 0px 2px 5px rgba(0,0,0,0.1); text-align: center; white-space: nowrap;">
  <b>{COPYRIGHT}</b> | <span style="color:#666;">{FONTE_DATI}</span> | <span style="color:#005580; font-weight:bold;">{STRINGA_AGGIORNAMENTO}</span>
</div>
'''
