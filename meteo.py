...
def genera_index_html():
    with open("index.html", "w", encoding="utf-8") as f:
        ...
        // 2. RIPRISTINO STATO AD OGNI CARICAMENTO DEL GIORNO (CON CONTROLLO DINAMICO)
        document.getElementById('frame').addEventListener('load', function() {
            ...
            // Creiamo un controllo continuo (ogni 50ms) per verificare quando le funzioni della mappa sono pronte
            var controlloProntezza = setInterval(function() {
                if (typeof iframeWindow.aggiornaMappaEInvolucri === 'function' && 
                    typeof iframeWindow.mostraRegioneLaterale === 'function') {
                    
                    // LE FUNZIONI SONO PRONTE! Blocchiamo il controllo continuo
                    ...
                    // --- B. RIPRISTINO TABELLA REGIONE (Niente più salti sulla Toscana!) ---
            ...
            }, 50); // Controlla ogni 50 millisecondi, è istantaneo
            
            // Sicurezza: se dopo 3 secondi non ha caricato, interrompiamo per non intasare la memoria
            setTimeout(function() { clearInterval(controlloProntezza); }, 3000);
        });
                }
